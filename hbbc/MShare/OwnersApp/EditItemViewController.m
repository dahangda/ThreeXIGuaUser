...
@interface EditItemViewController ()<HXPhotoViewDelegate,UITextViewDelegate,UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
...
@property (nonatomic,strong)NSArray *uesTimeArr;
//第一个标签
...
- (HXPhotoManager *)manager {
    if (!_manager) {
        ...
        _manager.photoMaxNum = 9 - _networkPhotos.count;
...
-(UITableView *)secondTableView
{
    if(!_secondTableView)
    {
        ...
        [_secondTableView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.left.equalTo(50);
            make.right.equalTo(-50);
            make.height.equalTo(38 * _uesTimeArr.count);
        }];
    }
...
- (void)viewDidLoad {
    ...
    _btnRight = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 100, 30)];
    [_btnRight addTarget:self action:@selector(deleteItems) forControlEvents:UIControlEventTouchUpInside];
    [_btnRight setTitle:@"删除物品" forState:UIControlStateNormal];
...
-(void)deleteItems
{
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"亲" message:@"您确定要删除此物品吗?" preferredStyle:UIAlertControllerStyleAlert];
    ...
    UIAlertAction *OKAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction *_Nonnull action){
            ...
            [[NetworkSingleton shareManager] httpRequest:parameters url:DELETEITEM success:^(id responseBody){
...
-(void)getGoodsTypeInfo
{
    ...
    [[NetworkSingleton shareManager] httpRequest:parameters url:GETITEMTYPE success:^(id responseBody){DHResponseBodyLog(responseBody);
        ...
        _uesTimeArr = [_labelOptionArr[1] componentsSeparatedByString:@","];
        _secondLabelArr = [_labelOptionArr[2] componentsSeparatedByString:@","];
        [self.rightLabelArr addObjectsFromArray:@[_typeArr[0],@"扫码使用",@"0元/次",@"0元",_firstLabelArr[0],_uesTimeArr[0],_secondLabelArr[0]]];
        [self getItemInfo];
...
-(void)getItemInfo
{
    ...
    [[NetworkSingleton shareManager] httpRequest:parameters url:OWNERITEMDETAIL success:^(id responseBody){
        ...
        [self initUI];
        _textView.text = responseBody[@"GoodsIntroduceText"];

        NSString *businessType  = [NSString stringWithFormat:@"%@",responseBody[@"BusinessType"]];
        ...
        NSString *GoodsUsePrice = [NSString stringWithFormat:@"%@元/次",responseBody[@"GoodsUsePrice"]];
        NSString *GoodsDeposit = [NSString stringWithFormat:@"%@元",responseBody[@"GoodsDeposit"]];
        [_rightLabelArr replaceObjectAtIndex:1 withObject:businessType];
        [_rightLabelArr replaceObjectAtIndex:2 withObject:GoodsUsePrice];
        [_rightLabelArr replaceObjectAtIndex:3 withObject:GoodsDeposit];
...
-(void)initUI
{
    ...
    self.manager.networkPhotoUrls = [NSMutableArray arrayWithArray:_networkPhotos];
...
-(void)commitJson
{
    ...
    [[NetworkSingleton shareManager] httpRequest:parameters url:UPDATEITEMINFO success:^(id responseBody){
        DHResponseBodyLog(responseBody);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        _selectCell = [tableView dequeueReusableCellWithIdentifier:@"typeTableViewCell"];
        _selectCell.label.text = _uesTimeArr[indexPath.row];
        _selectCell.selectionStyle = UITableViewCellSelectionStyleNone;
                if (indexPath.row == 0)
                {
                    _selectCell.btn.selected = YES;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if ([tableView isEqual:_typeTableView])
    {
        ...
        else
        {
            ...
            for (int i = 0; i <  _uesTimeArr.count; i++)
            {
            ...
            [self.rightLabelArr replaceObjectAtIndex:5 withObject:_uesTimeArr[0]];
            ...
            [self.secondTableView mas_updateConstraints:^(MASConstraintMaker *make) {
                make.height.equalTo(_uesTimeArr.count * 38);
            }];
            self.secondTableView.hidden = YES;
    ...
    else if ([tableView isEqual:_secondTableView])
    {
        ...
        _cell.rightLabel.text = _uesTimeArr[indexPath.row];
        for (int i = 0; i < _uesTimeArr.count; i++)
        {
